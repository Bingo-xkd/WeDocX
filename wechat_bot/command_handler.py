...
from wechat_bot.api_client import get_task_status, start_task

logger = get_bot_logger()

# 这里可以补充实际的命令处理函数定义，例如：
# async def handle_status(msg: Message, task_id: str):
#     try:
...
async def poll_task_status(msg: Message, task_id: str):
    while True:
        try:
            ...
            logger.error(
                f"Error polling status for Task ID {task_id}: {e}", exc_info=True
            )
            await msg.say(
                f"Error polling status for Task ID {task_id}: An internal error occurred."
            )
            break
