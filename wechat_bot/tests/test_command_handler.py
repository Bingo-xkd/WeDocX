...
async def test_handle_start_success(mock_message):
    ...
    with patch("wechat_bot.command_handler.start_task") as mock_start_task, patch(
        "wechat_bot.command_handler.asyncio.create_task"
    ) as mock_create_task:
...
async def test_poll_task_status_success(mock_message):
    ...
    with patch("wechat_bot.command_handler.get_task_status") as mock_get_status, patch(
        "wechat_bot.command_handler.asyncio.sleep", return_value=None
    ):
